...
export const Container = styled.TouchableOpacity.attrs({ activeOpacity: .9 }) <buttonProps>`
  border-radius: 100px;
  border:  1px solid ${({ theme, selected }) => selected ? theme.colors.background : transparentize(0.5, theme.colors.text)};
  padding: 10px 25px;
  margin-right: 10px;
...
export const Title = styled.Text<buttonProps>`
    color: ${({ theme, selected }) => selected ? theme.colors.primaryText : transparentize(0.5, theme.colors.text)};
    font-size: 14px;
